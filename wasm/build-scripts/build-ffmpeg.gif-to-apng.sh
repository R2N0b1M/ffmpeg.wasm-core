...
if [[ "$FFMPEG_ST" != "yes" ]]; then
  mkdir -p wasm/packages/core.gif-to-apng/dist
  EXPORTED_FUNCTIONS="[_main, _proxy_main]"
  EXTRA_FLAGS=(
    -pthread
    -s USE_PTHREADS=1                             # enable pthreads support
    -s PROXY_TO_PTHREAD=1                         # detach main() from browser/UI main thread
    -o wasm/packages/core.gif-to-apng/dist/ffmpeg-core.js
  )
else
  mkdir -p wasm/packages/core-st.gif-to-apng/dist
  EXPORTED_FUNCTIONS="[_main]"
  EXTRA_FLAGS=(
    -o wasm/packages/core-st.gif-to-apng/dist/ffmpeg-core.js
  )
fi
FLAGS=(
  ...
  -lavfilter -lavformat -lavcodec -lswresample -lswscale -lavutil -lm -lz
